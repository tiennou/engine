...
exports.make = function (_runtimeData, _intents, _register, _globals) {
    ...
    StructureNuker.prototype.launchNuke = register.wrapFn(function (pos) {
        ...
        if (runtimeData.rooms[this.room.name].novice > Date.now() || runtimeData.rooms[this.room.name].respawnArea > Date.now() || !_.isUndefined(runtimeData.roomStatusData.novice[pos.roomName]) || !_.isUndefined(runtimeData.roomStatusData.respawn[pos.roomName])) {
            return C.ERR_INVALID_TARGET;
        }
        if (this.cooldown > 0) {
            return C.ERR_TIRED;
        ...
        var _utils$roomNameToXY5 = utils.roomNameToXY(pos.roomName),
            _utils$roomNameToXY6 = _slicedToArray(_utils$roomNameToXY5, 2),
            tx = _utils$roomNameToXY6[0],
            ty = _utils$roomNameToXY6[1];

        var _utils$roomNameToXY7 = utils.roomNameToXY(data(this.id).room),
            _utils$roomNameToXY8 = _slicedToArray(_utils$roomNameToXY7, 2),
            x = _utils$roomNameToXY8[0],
            y = _utils$roomNameToXY8[1];
...
//# sourceMappingURL=../sourcemaps/game/structures.js.map
