...
function _getActiveBodyparts(body, type) {
    ...
    for (var i = body.length - 1; i >= 0; i--) {
        if (body[i].hits <= 0) break;
        if (body[i].type === type) count++;
    }
    return count;
}
...
exports.make = function (_runtimeData, _intents, _register, _globals) {
    ...
    Creep.prototype.harvest = register.wrapFn(function (target) {
        ...
        if (register.sources[target.id] && target instanceof globals.Source) {
            ...
            if (this.room.controller && (this.room.controller.owner && this.room.controller.owner.username != runtimeData.user.username || this.room.controller.reservation && this.room.controller.reservation.username != runtimeData.user.username)) {
                return C.ERR_NOT_OWNER;
            }
    ...
    Creep.prototype.transfer = register.wrapFn(function (target, resourceType, amount) {
        ...
        if (!target || !target.id || !register.spawns[target.id] && !register.powerCreeps[target.id] && !register.creeps[target.id] && !register.structures[target.id] && !register.customObjects[target.id] || !data(target.id).store && register.structures[target.id].structureType != 'controller' || target instanceof globals.Creep && target.spawning || !(target instanceof globals.StructureSpawn) && !(target instanceof globals.Structure) && !(target instanceof globals.Creep) && !(target instanceof globals.PowerCreep) && !register.customObjects[target.id]) {
            register.assertTargetObject(target);
            return C.ERR_INVALID_TARGET;
        }
    ...
    Creep.prototype.withdraw = register.wrapFn(function (target, resourceType, amount) {
        ...
        if (!target || !target.id || !data(target.id).store || (!register.structures[target.id] || !(target instanceof globals.Structure)) && !(target instanceof globals.Tombstone) && !(target instanceof globals.Ruin) && !register.customObjects[target.id]) {
            register.assertTargetObject(target);
            return C.ERR_INVALID_TARGET;
        }
    ...
    Creep.prototype.claimController = register.wrapFn(function (target) {
        ...
        if (controllersClaimed && (!runtimeData.user.gcl || runtimeData.user.gcl < utils.calcNeededGcl(controllersClaimed + 1))) {
    ...
    Creep.prototype.dismantle = register.wrapFn(function (target) {
        ...
        if (!target || !target.id || !register.structures[target.id] || !(target instanceof globals.Structure) && !(target instanceof globals.StructureSpawn) || !C.CONSTRUCTION_COST[target.structureType]) {
            register.assertTargetObject(target);
            return C.ERR_INVALID_TARGET;
        }
        if (!target.pos.isNearTo(this.pos)) {
...
//# sourceMappingURL=../sourcemaps/game/creeps.js.map
