...
exports.makePos = function (_register) {
    ...
    RoomPosition.prototype.findClosestByPath = register.wrapFn(function (type, opts) {
        ...
        if (opts.algorithm == 'dijkstra') {
            ...
            endNodes.objects.forEach(i => {
                var distance = this.isEqualTo(i) ? -1 : this.isNearTo(i) ? 0 : 1;
                if (distance < isNear) {
                    result = i;
                    isNear = distance;
                }
        ...
        if (opts.algorithm == 'astar') {
            ...
            endNodes.objects.forEach(i => {
                ...
                var path,
                    distance = this.isEqualTo(i) ? -1 : this.isNearTo(i) ? 0 : (path = this.findPathTo(i, opts)) && path.length > 0 && room.getPositionAt(path[path.length - 1].x, path[path.length - 1].y).isNearTo(i) ? path.length : undefined;
