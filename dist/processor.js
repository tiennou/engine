    ...
    return q.when().then(() => {
        ...
        _.forEach(roomObjects, object => {
            ...
            if (object.type == 'controller') {
                ...
                if (object.reservation && object.reservation.user == '2' && object.reservation.endTime - gameTime < C.CONTROLLER_RESERVE_MAX - C.INVADER_CORE_CONTROLLER_POWER * C.CONTROLLER_RESERVE) {
                    activateRoom = true;
                }
        ...
        for (let keeper of keepers) {
            ...
            _.forEach(i, (ii, objId) => {
                objectsIntents[objId] = _.assign(ii, objectsIntents[objId] || {});
            });
        }
        ...
        for (let invader of invaders) {
            ...
            _.forEach(i, (ii, objId) => {
                objectIntents[objId] = _.assign(ii, objectIntents[objId] || {});
            });
        }
        ...
        if (invaderCore && invaderCore.user) {
            ...
            _.forEach(i, (ii, objId) => {
                intents.users[invaderCore.user].objects[objId] = _.assign(ii, intents.users[invaderCore.user].objects[objId] || {});
            });
        }
        ...
        if (intents) {
            ...
            _.forEach(intents.users, (userIntents, userId) => {
                ...
                for (var objectId in userIntents.objects) {
                    ...
                    if (object.type == 'creep') require('./processor/intents/creeps/intents')(object, objectIntents, scope);
                    if (object.type == 'powerCreep') require('./processor/intents/power-creeps/intents')(object, objectIntents, scope);
                    if (object.type == 'link') require('./processor/intents/links/intents')(object, objectIntents, scope);
                    if (object.type == 'tower') require('./processor/intents/towers/intents')(object, objectIntents, scope);
                    if (object.type == 'lab') require('./processor/intents/labs/intents')(object, objectIntents, scope);
                    if (object.type == 'spawn') require('./processor/intents/spawns/intents')(object, objectIntents, scope);
                    ...
                    driver.config.emit('processObjectIntents', object, userId, objectIntents, roomObjects, roomTerrain, gameTime, roomInfo, bulk, bulkUsers);
                }
            });
        }
        ...
        _.forEach(roomObjects, object => {
            ...
            if (object.type == 'invaderCore') require('./processor/intents/invader-core/tick')(object, scope);
            if (object.type == 'energy') require('./processor/intents/energy/tick')(object, scope);
            if (object.type == 'source') require('./processor/intents/sources/tick')(object, scope);
            if (object.type == 'deposit') require('./processor/intents/deposits/tick')(object, scope);
            if (object.type == 'mineral') require('./processor/intents/minerals/tick')(object, scope);
            if (object.type == 'creep') require('./processor/intents/creeps/tick')(object, scope);
            if (object.type == 'powerCreep') require('./processor/intents/power-creeps/tick')(object, scope);
            if (object.type == 'spawn') require('./processor/intents/spawns/tick')(object, scope);
            if (object.type == 'rampart') require('./processor/intents/ramparts/tick')(object, scope);
            if (object.type == 'extension') require('./processor/intents/extensions/tick')(object, scope);
            if (object.type == 'road') require('./processor/intents/roads/tick')(object, scope);
            if (object.type == 'constructionSite') require('./processor/intents/construction-sites/tick')(object, scope);
            if (object.type == 'keeperLair') require('./processor/intents/keeper-lairs/tick')(object, scope);
            if (object.type == 'portal') require('./processor/intents/portals/tick')(object, scope);
            if (object.type == 'constructedWall') require('./processor/intents/constructedWalls/tick')(object, scope);
            if (object.type == 'link') require('./processor/intents/links/tick')(object, scope);
            if (object.type == 'extractor') require('./processor/intents/extractors/tick')(object, scope);
            if (object.type == 'tower') require('./processor/intents/towers/tick')(object, scope);
            if (object.type == 'controller') require('./processor/intents/controllers/tick')(object, scope);
            if (object.type == 'lab') require('./processor/intents/labs/tick')(object, scope);
            if (object.type == 'container') require('./processor/intents/containers/tick')(object, scope);
            if (object.type == 'terminal') require('./processor/intents/terminal/tick')(object, scope);
            if (object.type == 'tombstone') require('./processor/intents/tombstones/tick')(object, scope);
            if (object.type == 'ruin') require('./processor/intents/ruins/tick')(object, scope);
            if (object.type == 'factory') require('./processor/intents/factories/tick')(object, scope);
            ...
            if (object.user) {
                ...
                if (object.type != 'constructionSite' && !object.newbieWall && object.type != 'ruin' && object.type != 'tombstone' && (object.type != 'rampart' || !object.isPublic)) {
...
driver.connect('processor').then(() => driver.queue.create('rooms', 'read')).catch(error => {
    ...
    function loop() {
        ...
        roomsQueue.fetch().then(_roomId => {
            driver.config.emit('processorLoopStage', 'getRoomData', _roomId);
            roomId = _roomId;
            return q.all([driver.getRoomIntents(_roomId), driver.getRoomObjects(_roomId), driver.getRoomTerrain(_roomId), driver.getGameTime(), driver.getRoomInfo(_roomId), driver.getRoomFlags(_roomId)]);
        }).then(result => {
            ...
            processRoom(roomId, {
                ...
                roomInfo: result[4],
                flags: result[5]
            }).catch(error => console.log('Error processing room ' + roomId + ':', _.isObject(error) ? error.stack || error : error)).then(() => {
                return driver.clearRoomIntents(roomId);
            }).then(() => roomsQueue.markDone(roomId));
        }).catch(error => console.error('Error in processor loop:', _.isObject(error) && error.stack || error)).then(() => {
