...
const range = function (a, b) {
    if (_.isUndefined(a) || _.isUndefined(a.x) || _.isUndefined(a.y) || _.isUndefined(a.room) || _.isUndefined(b) || _.isUndefined(b.x) || _.isUndefined(b.y) || _.isUndefined(b.room) || a.room != b.room) {
        return Infinity;
    }
...
const deployStronghold = function deployStronghold(context) {
    ...
    if (core.deployTime && core.deployTime <= 1 + gameTime) {
        ...
        _.forEach(ramparts, rampart => {
            bulk.remove(rampart._id);delete roomObjects[rampart._id];
        });
...
const handleController = function reserveController(context) {
    const gameTime = context.gameTime,
          ...
          roomController = context.roomController;
...
const towersMaintenance = function towersMaintenance(context) {
    const intents = context.intents,
          towers = context.towers,
          ramparts = context.ramparts,
          damagedDefenders = context.damagedDefenders,
          damagedRoads = context.damagedRoads;
...
const maintainPopulation = function (context) {
    ...
    for (let i in core.population) {
        maintainCreep(`defender${i}`, creeps.bodies[core.population[i].body], context, creeps.behaviors[core.population[i].behavior]);
    }
};
...
const assignDefenders = function assignDefenders(context) {
    ...
    _.forEach(context.defenders, d => {
        if (_.some(d.body, { type: C.ATTACK })) {
            melees.push(d._id.toString());
        }
        if (_.some(d.body, { type: C.RANGED_ATTACK })) {
...
module.exports = {
    behaviors: {
        ...
        'bunker3': function (context) {
            ...
            if (!core.population) {
                bulk.update(core, {
                    population: [{ body: 'fullDefender', behavior: 'simple-melee' }, { body: 'fullDefender', behavior: 'simple-melee' }]
                });
            }
        ...
        'bunker4': function (context) {
            ...
            if (!core.population) {
                const populationDeck = [{ body: 'fortifier', behavior: 'fortifier' }, ...new Array(4).fill({ body: 'boostedDefender', behavior: 'coordinated' }), ...new Array(3).fill({ body: 'boostedRanger', behavior: 'coordinated' })];
        ...
        'bunker5': function (context) {
            ...
            if (!core.population) {
                const populationDeck = _.shuffle([{ body: 'fortifier', behavior: 'fortifier' }, ...new Array(7).fill({ body: 'fullBoostedMelee', behavior: 'coordinated' }), ...new Array(9).fill({ body: 'fullBoostedRanger', behavior: 'coordinated' })]);

                bulk.update(core, {
                    population: [{ body: 'fortifier', behavior: 'fortifier' }, ..._(populationDeck).shuffle().take(8).value()]
                });
            }
...
//# sourceMappingURL=../../../../sourcemaps/processor/intents/invader-core/stronghold/stronghold.js.map
