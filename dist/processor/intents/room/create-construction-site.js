...
module.exports = function (userId, intent, { roomObjects, roomTerrain, bulk, roomController }) {
    ...
    if (!utils.checkConstructionSite(roomObjects, intent.structureType, intent.x, intent.y) || !utils.checkConstructionSite(roomTerrain, intent.structureType, intent.x, intent.y)) {
        return;
    }
    ...
    if (intent.structureType == 'road') {
        if (_.any(roomObjects, { x: intent.x, y: intent.y, type: 'swamp' }) || utils.checkTerrain(roomTerrain, intent.x, intent.y, C.TERRAIN_MASK_SWAMP)) {
            progressTotal *= C.CONSTRUCTION_COST_ROAD_SWAMP_RATIO;
        }
        if (_.any(roomObjects, { x: intent.x, y: intent.y, type: 'wall' }) || utils.checkTerrain(roomTerrain, intent.x, intent.y, C.TERRAIN_MASK_WALL)) {
...
//# sourceMappingURL=../../../sourcemaps/processor/intents/room/create-construction-site.js.map
