...
module.exports = function ({ orders, userIntents, usersById, gameTime, roomObjectsByType, bulkObjects,
    ...
    function executeTransfer(fromTerminal, toTerminal, resourceType, amount, transferFeeTerminal, additionalFields) {
        ...
        if (transferFeeTerminal === fromTerminal && (resourceType != C.RESOURCE_ENERGY && fromTerminal.store.energy < transferCost || resourceType == C.RESOURCE_ENERGY && fromTerminal.store.energy < amount + transferCost) || transferFeeTerminal === toTerminal && toTerminal.store.energy < transferCost) {
            return false;
        }
        ...
        userIntents.forEach(iUserIntents => {
            ...
            if (iUserIntents.intents.createOrder) {
                iUserIntents.intents.createOrder.forEach(intent => {
                    ...
                    if (!_.contains(C.INTERSHARD_RESOURCES, intent.resourceType) && (!terminalsByRoom[intent.roomName] || terminalsByRoom[intent.roomName].user != iUserIntents.user)) {
                        return;
                    }
    ...
    if (orders) {
        orders.forEach(order => {
            ...
            if (order.type == C.ORDER_SELL) {
                ...
                var availableResourceAmount = _.contains(C.INTERSHARD_RESOURCES, order.resourceType) ? (usersById[order.user].resources || {})[order.resourceType] || 0 : terminal && terminal.user == order.user ? terminal.store[order.resourceType] || 0 : 0;
    ...
    return () => q.all([bulkUsers.execute(), bulkMarketOrders.execute(), bulkMarketIntershardOrders.execute(), bulkUsersMoney.execute(), bulkTransactions.execute(), bulkUsersResources.execute(), driver.clearMarketIntents()]);
};
//# sourceMappingURL=../../sourcemaps/processor/global-intents/market.js.map
